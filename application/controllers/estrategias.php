...
class Estrategias extends CI_Controller{
	...
	public function index(){
		...
		if($this->input->post('edit_estrategia')){
			$config=array(
				array(
                 'field'   => 'nombre',
			...
			if ($this->form_validation->run()){
				...
				$estrategia=$this->config_estrategia_update();
				$this->model_estrategias->update('estrategias',$estrategia,$id_estrategia);
			}
		}
		if($this->input->post('edit_imagen')){
			$upload_data=$this->upload_img();
			if(isset($upload_data['upload']['error'])){
				$data['control']=$upload_data;
			}
			else{
				$id_estrategia=$this->input->post('id_estrategia');
				$estrategia=array(
					'imagen'=>$upload_data['upload']['upload_data']['file_name']
				);
				$this->model_estrategias->update('estrategias',$estrategia,$id_estrategia);
			}
		}
		if($this->input->post('crear_estrategia')){
			$config=array(
				array(
                 'field'   => 'nombre',
                 'label'   => 'Nombre de la estrategia',
			...
			$this->form_validation->set_rules($config);
			if ($this->form_validation->run()){
				$estrategia=$this->config_estrategia_insert();
				$this->model_estrategias->insert('estrategias', $estrategia);
			}
		}
		

		$user=$this->model_usuarios->get_by_id('usuarios',$this->session->userdata('id'));
		$etapas=$this->model_etapas->get_all('etapas');
		foreach ($etapas as $key => $etapa) {
			$etapa->estrategias=$this->model_estrategias->get_by_etapa($this->session->userdata('id_organizacion'),$etapa->id);
		}
		$data['etapas']=$etapas;
		$data['nombre']='Estratégias';
		$data['view']='estrategias/show_all';
		$this->load->view('template/body',$data);
	}

	public function config_estrategia_insert(){
		$data=array(
	...
	public function upload_img(){
		...
		$id_org=$this->session->userdata('id_organizacion');
		$id_estrategia=$this->input->post('id_estrategia');
		$config['file_name'] = 'thumbnail_'.$id_org.'_'.$id_estrategia;
		$config['upload_path'] = './assets/img/estrategias/thumbnails/';
	...
	public function config_estrategia_update(){
		if(!$this->input->post('edit_estrategia')){
			show_404();
		}
		$data=array(
			'nombre'=>$this->input->post('nombre'),
			'descripcion'=>$this->input->post('descripcion'),
			'id_etapa'=>$this->input->post('etapa')
		);
		return $data;
	}

	public function edit($id_estrategia){
		$estrategia=$this->model_estrategias->get_by_id('estrategias',$id_estrategia);
		$data['estrategia']=$estrategia[0];

		$data['nombre']='Editar estrategia: '.$estrategia[0]->nombre;
		$data['view']='estrategias/edit';
		$this->load->view('template/body',$data);
	}
